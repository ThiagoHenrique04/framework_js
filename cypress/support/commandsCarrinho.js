...
cy.get(loc.HOME.PRODUCT_NAME).eq(0).invoke('text').as('productName1')
cy.get(loc.HOME.PRODUCT_NAME).eq(1).invoke('text').as('productName2')
...
cy.get(loc.CHECKOUT.ITEMS_PRICE).then($prices =>{
    ...
    cy.log(`Subtotal: $${expectedSubtotal.toFixed(2)}, Tax: $${tax.toFixed(2)}, Expected Total: $${expectedTotal.toFixed(2)}`)
})
})
})
